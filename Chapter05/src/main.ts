    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                ...
                Seminar.seminarsByYear(data, year);
                break;
            case 2:
                const title: string = readline.question("세미나 제목을 입력하세요: ");
                const seminarDetails: Seminar | undefined = Seminar.getSeminarDetails(data, title);
                if (seminarDetails) {
                ...
                break;
            case 3:
                const newSeminar: DetailSchedule = Seminar.createSeminar();
